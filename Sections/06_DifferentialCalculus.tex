...
To study the variations close to \(a\), we consider the limit of this ratio as \(h\) tends to zero. See figure \ref{fig:Ch06-derivative-of-f-as-limit}.
%
\[
...
\begin{figure}[H]
    ...
    \label{fig:Ch06-derivative-of-x-squared-as-tangent}
\end{figure}
...
Recall that a function \(f(x)\) is said to be differentiable in \(a\) if the derivative of \(f'(x)\) exists when \(x = a\).
...
There are several ways to distinguish between these cases, as will be demonstrated using the following function.
%
\[f(x) = 2x^3 - 9x^2 + 12x - 4\]
%
