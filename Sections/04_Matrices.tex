...
We define \(f\) to be a linear map that scales the length of a vector by a factor of \(2\) and then rotates it by \(30\degree\). See figure \ref{fig:Ch04-example-1}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.4]
        ...
        \draw[thick, ->, >=latex] (-8,0)--(8,0) node[right]{\(x\)};
        \draw[thick, ->, >=latex] (0,-8)--(0,8) node[above]{\(y\)};
    ...
    \begin{tikzpicture}[scale=0.4]
        ...
        \foreach \x in {-3,...,3} {
            \foreach \y in {-3,...,3} {
                \draw[line width=1.5pt, Fuchsia, -stealth, draw opacity=0.3] (0,0) -- ($2*({cos(30) * \x + cos(120) * \y}, {sin(30) * \x + sin(120) * \y})$);
            }
        }
    \end{tikzpicture}
    
    \caption{The linear map \(f\) takes a vector (in orange) and scales it by a factor of \(2\) before rotating it by \(30\degree\) anticlockwise. This results in a new vector (in purple).}
...
\vspace{15pt}
\begin{mdframed}[linewidth=1pt]
\noindent \textbf{Setup.} Like the previous example, we set both \(U\) and \(V\) to \(\mathbb{R}^2\), and we want to find a matrix \(M\) that corresponds to a linear map \(f\) where vectors are scaled \(2\) times and rotated by \(30\degree\) anticlockwise.
...
\begin{itemize}
    ...
    \item The \textit{cofactor} of this element, denoted as \(C_{i,j}\), is exactly the same as its \textit{minor}, except the sign (positive or negative) may has to be flipped depending on its position.
    %
    \[C_{i,j} = (-1)^{i + j} M_{ij}\]
